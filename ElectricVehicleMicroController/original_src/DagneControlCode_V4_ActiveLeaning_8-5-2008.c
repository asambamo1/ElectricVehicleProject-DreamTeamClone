...
void SampleSensors(void)
{
	...
	{
		//  Compute On time	
		OnTime = (long int)SteerSenseLatch1 + ((long int)SteerCntOvrFlowLatch << 16 - 1) - (long int)SteerSenseLatch0; // Should always be positive
		//  Compute period
		Period = (long int)SteerSenseLatch2 + ((long int)SteerCntOvrFlowLatch << 16 - 1) - (long int)SteerSenseLatch0;
			
		SteerSenseFlag = 0; // Reset flag so that more data can be taken
		
		SteerAngle = -360.0 * ((float) OnTime/ ((float) Period) - 0.5);
	}
	...
	if (LeanSenseFlag == 1) 
	{
		...
		LeanAngle = 360.0 * ((float) OnTime/ ((float) Period) - 0.5);
	}

...
#define STEER_GAIN			19.1			//  Roughly a 25deg error maps to 255.0, or full open valve
#define LEAN_GAIN			25.0
#define BRAKE_GAIN			30			//  Roughly 1500psi error yields full open valve

#define STEER_MAX_ANGLE     25.0  // Maximum steering angle (at low speeds)
#define STEER_MIN_ANGLE 	10.0  // Minimum steering angle (at high speeds)

#define SSS_MAX_VEL 		10.0  // Speed Sensitive Steering (SSS) max velocity
#define SSS_MIN_VEL 		3.0   // Speed Sensitive Steering (SSS) min velocity
...
#define MPS2MPH				2.236936    //  Conversion constant v_mph = MPS2MPH*v_mps
#define SPEED_SENSOR_GAIN   0.009425    //  Sensor constant [meters per cycle], 9.5mm distance between two rising edges on sensor
#define	SPEED_SENSOR_CNT2VEL  57726.765   //  Sensor constant mapping measured counts/cycle to m/s
#define MAX_SPEED_MPH		20.0		//  Maximum speed in mph
#define VEL_MIN				0.3			//  Speed below which direction can change
#define MAX_BRAKE_PRESSURE  700.0      //  Maximum cylinder pressure in PSI


#define AB_DEADZONE			0.03		//  Accel/Brake deadzone in [0,1]
#define AB_SCALE 		   	1.0309 // AB_SCALE = 1/(1-AB_DEADZONE);

#define LR_DEADZONE			0.02		//  Left/Right deadzone in [0,1]
#define LR_SCALE 		   	1.0204 // LR_SCALE = 1/(1-LR_DEADZONE);
...
#define STEER_PWM_LIMIT     90      //  Rate limit Steering
...
#define JSLR_FILT_A         0.9450		// Joystick Left/Right filter matrix/gain A
#define JSLR_FILT_B         0.0550      // Joystick Left/Right filter matrix/gain B
...
sbit NeutralBtn = P2^5;
sbit Joystick_Btn0 = P2^6;
sbit Joystick_Btn1 = P2^7;
...
xdata float SteerAngleState;
xdata float LeanAngleState;
xdata float BrakePressState;
xdata float VelState;
xdata float TestState;

xdata unsigned char DriveState;

xdata float SSS_Slope; 
xdata float SSS_Offset; 

xdata unsigned char DataSampleCntr;
...
void main (void) {
   ...
   while (1) {                            // spin forever
	...
	{	
		...
		SteerAngleState = -SteerAngleAccum / ((float) SteerAngleAccumCnt);
		SteerAngleAccum = 0;
		SteerAngleAccumCnt = 0;

		//  Velocity Measutement
		VelState = SpeedAccum/ ((float)SpeedAccumCnt);
		SpeedAccum = 0;
		SpeedAccumCnt = 0;
		...
		ComputePWMOutputs();
		//  Steer Control 
		//SteerRef = MPS2MPH*SpeedAccum/ ((float)SpeedAccumCnt);

		ControlVal = -STEER_GAIN*(SteerRef - SteerAngleState);
		
		//ControlVal = PWM_RESOLUTION*SteerRef/STEER_MAX_ANGLE;  //  Map joystick pos to PWM dutycycle
		
		if (ControlVal > STEER_PWM_LIMIT){SteerValveCmd = STEER_PWM_LIMIT; } 
		else if (ControlVal < -STEER_PWM_LIMIT){SteerValveCmd = -STEER_PWM_LIMIT; } 
		...
		{
			AccelRef = 0.1 * AccelRef;  // Cut the throttle back big time 
		}
		else
		{
			AccelRef = 4095.0*AccelRef;
		}
		*/

		ComputePWMOutputs();
		//  Clamp command to allowable range
		if (AccelRef > 4095.0) { AccelRef = 4095.0;}
		if (AccelRef < 0.0) { AccelRef = 0.0;}

		SFRPAGE   = DAC0_PAGE;
		AccelCmd = 4095 - ((unsigned int) AccelRef); // Must invert to cancel inverting OpAmp output stage
		...
		if (DataSampleCntr >= 20)
		{
			DataSampleCntr = 0;
		//  Transmit data for logging
			TestState ++;
			printf("Hi There\n"); //printf("V, %f\n", TestState);
		}
		else
...
BrakeRef = 0.0;
AccelRef = 0.0;
DynMaxSteerAngle = STEER_MAX_ANGLE;
...
SSS_Slope = (STEER_MIN_ANGLE - STEER_MAX_ANGLE)/(SSS_MAX_VEL - SSS_MIN_VEL);
SSS_Offset = STEER_MAX_ANGLE - SSS_Slope*SSS_MIN_VEL;
...
void SpeedSteeringControlMap(void)
{
	//float Joystick_LR_Ref = 0, 
	float Uin = 0.0, Ufb = 0.0, Joystick_FB = 0.0, VelStateLoc = 0.0;
	int tmpV;
	unsigned char DriveState_Next = 0, nDirChangeBtn = 0;

	SFRPAGE = CONFIG_PAGE;

	tmpV = 1729 - (int) SampleVoltage(0);
	//Joystick_FB_Ref = ((float) tmpV)/1642.0;  //  Sample JoystickFB
	Ufb = ((float) tmpV)/1642.0;  //  Sample JoystickFB

	ComputePWMOutputs();
	tmpV = 1672 - (int) SampleVoltage(1);
	//  Low Pass Filter Joystick input
	//Joystick_LR_Ref = ((float) tmpV)/1623.0;  //  Sample JoystickLR
	Uin = ((float) tmpV)/1623.0; //  LR joystick input, run through sat and deadzone first, then filter

	
	ComputePWMOutputs();
	if (Uin <= - LR_DEADZONE)
	{
		Uin = (Uin + LR_DEADZONE) * LR_SCALE;
	}
	else if (Uin > -LR_DEADZONE && Uin < LR_DEADZONE)
	{
		Uin = 0.0;  // Insert steering deadzone
	}
	else
	{
		Uin = (Uin - LR_DEADZONE) * LR_SCALE;
	}
	//  Update Joystick_LR_Ref filter
	//Joystick_LR_Ref = JSLR_FILT_A*Joystick_LR_Ref + JSLR_FILT_B*Uin;  //  Sample JoystickLR

	Joystick_LR_Ref = Uin;



	//  Speed Sensitive Steer/Lean Code
	//  Note: nDriveChangeBtn is active low, 0- active, 1- inactive
	nDirChangeBtn = Joystick_Btn1 && Joystick_Btn0;

	//DynMaxSteerAngle = SSS_Slope*VelState + SSS_Offset;
	//if (DynMaxSteerAngle > STEER_MAX_ANGLE) { DynMaxSteerAngle = STEER_MAX_ANGLE;}
	//else if (DynMaxSteerAngle < STEER_MIN_ANGLE) { DynMaxSteerAngle = STEER_MIN_ANGLE;}

	LeanRef = 0.0*LEAN_MAX_ANGLE; 
	SteerRef = Joystick_LR_Ref * STEER_MAX_ANGLE ;//DynMaxSteerAngle;  //  For now map joystick to steer only

	// Fwd-Reverse (For reverse, make a backward pull on joystick be rev accel)
	
  	//  Map joystick measurement to [-1,1] with a deadzone about zero
	if (Ufb > 1.0) {
		Joystick_FB = 1.0;
	}
	else if (Ufb >= AB_DEADZONE && Ufb <= 1.0){
		Joystick_FB = (Ufb - AB_DEADZONE) * AB_SCALE; 
	}
	else if (Ufb < AB_DEADZONE && Ufb > -AB_DEADZONE) {
		Joystick_FB = 0.0; 
	}
	else if (Ufb <= -AB_DEADZONE && Ufb >= -1.0) {
		Joystick_FB = (AB_DEADZONE + Ufb) * AB_SCALE; 
	}
	else 
	{ Joystick_FB = -1.0; 
	}

	VelStateLoc = VelState;	

	//============= DriveState System Behavior : State Transition FSM ==========
	//  Drive State Map
	//  0 - Park State :           Brakes on, throttle disabled
	//  1 - Park Leaving State :   Brakes on, throttle disabled
	//  2 - Forward Drive State :  Joystick back = brake, Joystick fwd = throttle
	//  3 - Reverse Drive State :  Joystick fwd = brake, Joystick back = throttle
	//  4 - Neutral State :        Joystick back = brake, throttle disabled

	

	if (DriveState == 0) // Park State
	{
		// Compute Next State
		if (nDirChangeBtn == 0 && Joystick_FB == 0.0 ) { DriveState_Next = 1; }
		//else if (NeutralBtn == 1) { DriveState_Next = 4; }
		else { DriveState_Next = 0;} 
		
		// Map Outputs 
		pKeySwitch = 0; // Throttle Disabled
		pRevSw1 = 1;  //  Forward motor direction
		pRevSw2 = 0;
		AccelRef = 0.0;
		BrakeRef = MAX_BRAKE_PRESSURE;
	}
	else if (DriveState == 1) // Park Leaving State
	{
		// Compute Next State
		if (nDirChangeBtn == 1) { DriveState_Next = 0; } // Return back to park state
		else if (nDirChangeBtn == 0 && Joystick_FB > 0.0) { DriveState_Next = 2;} // Forward state
		else if (nDirChangeBtn == 0 && Joystick_FB < 0.0) { DriveState_Next = 3;} // Reverse state
		else { DriveState_Next = 1; }
	...
	else if (DriveState == 2) // Forward State
	{
		// Compute Next State
		//if (((VelStateLoc < VEL_MIN) && (nDirChangeBtn == 0)) && Joystick_FB < 0.0) { DriveState_Next = 3; } // Reverse State
		if (VelStateLoc < VEL_MIN && 
			(nDirChangeBtn == 1 || (nDirChangeBtn == 0 && Joystick_FB == 0.0)))
			{ DriveState_Next = 0; } // Park State
		else { DriveState_Next = 2;}


		// Map Outputs 
		pKeySwitch = 1; // Throttle Enabled
		pRevSw1 = 1;  //  Forward motor direction
		pRevSw2 = 0;
		if (Joystick_FB >= 0) { 
			AccelRef = 4095.0*Joystick_FB; 
			BrakeRef = 0.0; 
		}
		else { 
			AccelRef = 0.0; 
			BrakeRef = -Joystick_FB * MAX_BRAKE_PRESSURE;
		}
	}
	else if (DriveState == 3) // Reverse State
	{
		// Compute Next State
		// if (VelStateLoc < VEL_MIN && nDirChangeBtn == 0 && Joystick_FB > 0.0){ DriveState_Next = 2;} // Forward State
		if (VelStateLoc < VEL_MIN && 
			(nDirChangeBtn == 1 || (nDirChangeBtn == 0 && Joystick_FB == 0.0)))
			{ DriveState_Next = 0; } // Park State 
		else { DriveState_Next = 3;}

		// Map Outputs 
		pKeySwitch = 1; // Throttle Enabled
		pRevSw1 = 0;  //  Reverse motor direction
		pRevSw2 = 1;
		if (Joystick_FB <= 0) { 
			AccelRef = -2000.0*Joystick_FB; //  Reduce throttle for reverse
			BrakeRef = 0.0; 
		}
		else { 
			AccelRef = 0.0; 
...
/*
	else if (DriveState == 4) // Neutral State
	{
		// Compute Next State
		if ( NeutralBtn == 0) { DriveState_Next = 0; }
		else { DriveState_Next = 4;}
...
*/
	...
	{
		DriveState_Next = 0; // If for some reason this happens return to Park State
	}

	DriveState = DriveState_Next;
...
unsigned int SampleVoltage(unsigned char ACh)
{
    ...
    result = sum>>1;   // divide by 2 and cast to uchar
...
void SampleEncoders(void)
{
	...
	if (CntOvrFlow_Speed <= 100)
	{
		if (SpeedSensFlag >= 2)
		{
			...
			if (Diff1 > 0) 
			{
				SpeedAccum = SPEED_SENSOR_CNT2VEL/ ((float)Diff1);
				SpeedAccumCnt = 1;
				
			}
			// ELSE: If Diff1 == 0, then we got two edges before our 600kHz clock counted once
			// which is an error condition, just keep previous value
			SpeedSensOvrFlow1 = 0;
			CntOvrFlow_Speed = 0;
			SpeedSensFlag = 0;  // Reset for another measurement
		}
	}
	else  //  No tooth event in nearly 1 sec, we are stopped
	{
		SpeedAccum = 0;
		SpeedAccumCnt = 1;	
		SpeedSensOvrFlow1 = 0;
...
*/
	...
	if (CCF2) // Check CCF2 bit 2 of PCA0CN, Speed sensor
	{
		...
		else if (SteerSensFlag == 1)
		{
			...
			SpeedSensOvrFlow1 = CntOvrFlow_Speed;  // Log number of overflows since last edge
		}
